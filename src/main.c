...
typedef struct http_state_s http_state_t;

struct http_state_s {
  unsigned last_allocated_stream_id;
  // TODO - what other data is necessary?
};
...
void echo_read(uv_stream_t *client, ssize_t nread, const uv_buf_t* buf) {
  fprintf(stderr, "Read from stream\n");
  if (nread < 0) {
...
int main() {
  ...
  // TODO store reference to loop in server?
  // server.data = ??
